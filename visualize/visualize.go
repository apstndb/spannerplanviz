...
func RenderImage(rowType *spannerpb.StructType, queryStats *spannerpb.ResultSetStats, format graphviz.Format, writer io.Writer, param VisualizeParam) error {
	g := graphviz.New()
	graph, err := g.Graph()
	if err != nil {
		return err
...
func buildGraphFromQueryPlan(graph *cgraph.Graph, rowType *spannerpb.StructType, queryStats *spannerpb.ResultSetStats, param VisualizeParam) error {
...
func renderTree(graph *cgraph.Graph, rowType *spannerpb.StructType, childLink *spannerpb.PlanNode_ChildLink, qp *queryplan.QueryPlan, param VisualizeParam) (*cgraph.Node, error) {
...
func isRemoteCall(node *spannerpb.PlanNode, cl *spannerpb.PlanNode_ChildLink) bool {
...
func renderNode(graph *cgraph.Graph, rowType *spannerpb.StructType, childLink *spannerpb.PlanNode_ChildLink, queryPlan *queryplan.QueryPlan, param VisualizeParam) (*cgraph.Node, error) {
...
func renderExecutionStatsOfNode(planNode *spannerpb.PlanNode, param VisualizeParam) string {
...
func setupQueryNode(graph *cgraph.Graph, queryStats *spannerpb.ResultSetStats, param VisualizeParam) (*cgraph.Node, error) {
...
func setupGvNode(graph *cgraph.Graph, planNode *spannerpb.PlanNode, nodeTitle string, metadataStr string) (*cgraph.Node, error) {
...
func getNodeTitle(planNode *spannerpb.PlanNode) string {
...
func isInlined(nodes []*spannerpb.PlanNode, node *spannerpb.PlanNode) bool {
	return node.GetKind() == spannerpb.PlanNode_SCALAR && (len(node.GetChildLinks()) == 0 || nodes[node.GetChildLinks()[0].GetChildIndex()].GetKind() != spannerpb.PlanNode_RELATIONAL)
}

func renderSerializeResult(rowType *spannerpb.StructType, childLinks []*childLinkGroup) string {
...
func getScalarChildLinks(qp *queryplan.QueryPlan, node *spannerpb.PlanNode, filter func(link *spannerpb.PlanNode_ChildLink) bool) []*childLinkGroup {
	...
	for _, cl := range node.GetChildLinks() {
		...
		if !filter(cl) || childNode.GetKind() != spannerpb.PlanNode_SCALAR {
			continue
		}
...
func getNonVariableChildLinks(plan *queryplan.QueryPlan, node *spannerpb.PlanNode) []*childLinkGroup {
	return getScalarChildLinks(plan, node, func(node *spannerpb.PlanNode_ChildLink) bool {
		return node.GetVariable() == ""
	})
}

func getVariableChildLinks(plan *queryplan.QueryPlan, node *spannerpb.PlanNode) []*childLinkGroup {
	return getScalarChildLinks(plan, node, func(node *spannerpb.PlanNode_ChildLink) bool {
		return node.GetVariable() != ""
	})
}
